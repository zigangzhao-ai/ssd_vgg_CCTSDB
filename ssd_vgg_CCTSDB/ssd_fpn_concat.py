...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        self.yb_256_1 = YB(256)
        self.yb_256_2 = YB1(256)
        self.yb_512 = YB1(512)
        # =====bobo新增==================
        # conv9_2到conv9_2
        self.conv256_512_1 = nn.Conv2d(256, 256, kernel_size=3, padding=1, stride=1)
        # conv9_2到conv8_2
        self.upsample_256_256_1 = Upsample(10)
        ...
        ##con8_2--yb(640-->256)
        self.conv_640_256 = nn.Conv2d(in_channels=640, out_channels=256, kernel_size=1, stride=1)

        #conv8_2-->smooth
        self.conv256_512_2 = nn.Conv2d(256, 512, kernel_size=3, padding=1, stride=1)

        
        ##add后512-->1024 conv8_2到conv7 反卷积上采样，尺度大一倍
        self.upsample_256_256_2 = Upsample(19)
        self.conv_512_1024 = nn.Conv2d(in_channels=512, out_channels=1024, kernel_size=1, stride=1)

        self.conv_1152_512 = nn.Conv2d(in_channels=1152, out_channels=512, kernel_size=1, stride=1)
        # conv7到conv7  尺度不变
        self.conv_1024_1024_1 = nn.Conv2d(in_channels=1024, out_channels=1024, kernel_size=1, stride=1)
        self.conv_512_1024_1 = nn.Conv2d(in_channels=512, out_channels=1024, kernel_size=3, padding=1, stride=1)
        # conv7 到 conv4_3  反卷积上采样，尺度大一倍
        self.upsample_512_512 = Upsample(38)
        ...
        self.conv_640_512_2 = nn.Conv2d(in_channels=640, out_channels=512, kernel_size=1,  stride=1)
        self.conv_512_1024_2 = nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, padding=1, stride=1)

        # 平滑层
        self.smooth = nn.Conv2d(256, 256, kernel_size=3, padding=1, stride=1)
        # self.smooth1 = nn.Conv2d(1024, 1024, kernel_size=3, kernel_size=3, padding=1, stride=1)
    ...
    def forward(self, x):
        ...
        con9_2_fp1 = self.yb_256_1(sources[5])

        conv9_2_fp = self.conv256_512_1(con9_2_fp1)
   
        # conv8_2层融合结果  self.bn1(self.conv1(x)) 在通道维度上融合
        conv8_fp1 = torch.cat((self.conv_256_128(self.bn(self.upsample_256_256_1(con9_2_fp1))), self.conv_512_512_1(sources[4]))
                               ,1)
        conv8_fp2 = self.conv_640_256(conv8_fp1)
        conv8_fp3 = self.yb_256_2(conv8_fp2)
        conv8_fp = self.conv256_512_2(conv8_fp3)
  
        # conv7层融合结果
        fc7_fp1 =  torch.cat((self.conv_256_128(self.bn(self.upsample_256_256_2(conv8_fp3))), self.conv_1024_1024_1(sources[3]))
                               ,1)
        fc7_fp2 = self.conv_1152_512(fc7_fp1)
        fc7_fp3 = self.yb_512(fc7_fp2)
        fc7_fp = self.conv_512_1024_1(fc7_fp3)
     
        # conv4_3层融合结果
        conv4_fp = torch.cat((self.conv_256_128(self.conv_512_256(self.bn1(self.upsample_512_512(fc7_fp2)))), self.conv_512_512_2(sources[1]))
                               ,1)
        conv4_fp  = self.conv_640_512_2(conv4_fp)
        conv4_fp = self.conv_512_1024_2(conv4_fp)
        ...
        if USE_CBAM:
            ...
            sources_final.append(self.CBAM4(conv9_2_fp))
        ...
        if USE_SE:
            ...
            sources_final.append(self.SE4(conv9_2_fp))
            ...
            sources_final.append(conv9_2_fp)
            sources_final.append(self.smooth(sources[6]))
            sources_final.append(self.smooth(sources[7]))
...
def YB(n):
    return nn.Sequential(
        nn.Conv2d(in_channels=n, out_channels=2*n, kernel_size=1, stride=1),
...
def multibox(vgg, extra_layers, cfg, num_classes):
    ...
    for k, v in enumerate(vgg_source):
        ...
        loc_layers += [nn.Conv2d(in_channels,
                                 cfg[k] * 4, stride=1, kernel_size=1)]
        conf_layers += [nn.Conv2d(in_channels,
                        cfg[k] * num_classes, stride=1, kernel_size=1)]
    # [x::y] 从下标x开始，每隔y取值
